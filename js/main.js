...
const ICONES = {
  ...
  globe: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a13 13 0 0 1 0 18M12 3a13 13 0 0 0 0 18"/></svg>',
  search: '<svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>',
  arrow: '<svg viewBox="0 0 24 24"><path d="M5 12h14M13 6l6 6-6 6"/></svg>',
  truck: '<svg viewBox="0 0 24 24"><path d="M2 7h11v9H2z"/><path d="M13 10h4l3 3v3h-7z"/><circle cx="6" cy="18" r="1.6"/><circle cx="17" cy="18" r="1.6"/></svg>',
  scale: '<svg viewBox="0 0 24 24"><path d="M12 3v18M5 7h14M5 7 3 12a2.5 2.5 0 0 0 5 0L5 7Zm14 0-2 5a2.5 2.5 0 0 0 5 0l-2-5Z"/></svg>',
  bolt: '<svg viewBox="0 0 24 24"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8Z"/></svg>',
  wallet: '<svg viewBox="0 0 24 24"><rect x="3" y="6" width="18" height="13" rx="2"/><path d="M3 10h18M16 14h.01"/></svg>',
...
function classeBadgeIndustria(industria = '') {
  ...
  if (i.includes('agricultura') || i.includes('pecuária') || i.includes('pecuaria')) return 'badge-ind-agricultura';
  if (i.includes('tecnologia') || i.includes('comunicação') || i.includes('comunicacao')) return 'badge-ind-tecnologia';
  if (i.includes('ensino') || i.includes('bds')) return 'badge-ind-ensino';
  if (i.includes('logística') || i.includes('logistica') || i.includes('supply')) return 'badge-ind-logistica';
  if (i.includes('construção') || i.includes('construcao')) return 'badge-ind-construcao';
...
function coverComFallback(slug, classeExtra = '') {
  ...
  const candidatosCover = extensoes.map(ext => imagemEmpresa(slug, `cover.${ext}`));
  const candidatosLogo = extensoes.map(ext => imagemEmpresa(slug, `logo.${ext}`));
...
function imgComFallback(slug, baseNome, extensoes = ['jpg', 'png', 'jpeg']) {
  const candidatos = extensoes.map(ext => imagemEmpresa(slug, `${baseNome}.${ext}`));
...
async function renderizarGridEmpresas(containerId, { limite = null } = {}) {
  ...
  if (empresas.length === 0) {
    container.innerHTML = '<p class="text-body-md text-muted">Nenhuma empresa encontrada.</p>';
    return;
  }
...
async function renderizarPerfilEmpresa() {
  ...
  if (!slug) {
    root.innerHTML = '<div class="container" style="padding: var(--space-xxl) 0;"><p class="text-body-lg">Empresa não especificada.</p></div>';
    return;
  }
  ...
  if (!empresa) {
    root.innerHTML = '<div class="container" style="padding: var(--space-xxl) 0;"><p class="text-body-lg">Empresa não encontrada.</p></div>';
    return;
  }
  ...
  const labels = stats.labels || ['Anos de Experiência', 'Volume de Produção', 'Volume de Vendas', 'Clientes Activos'];
      ...
      <div class="container" style="display:grid; grid-template-columns: 1fr 320px; gap: var(--space-lg); align-items:start;">
        ...
        <div>
          ...
          <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: var(--space-md);">
            ${servicos.map((s, i) => `
              <div class="card">
                <div style="height:140px; background-color: var(--color-surface-container);">
...
async function renderizarServicosDestaque(containerId, { limite = 4 } = {}) {
  ...
  if (servicosTodos.length === 0) {
    container.innerHTML = '<p class="text-body-md text-muted">Nenhum serviço encontrado.</p>';
    return;
  }
...
async function actualizarStatsHome() {
  ...
  const nServicos = empresas.reduce((soma, e) => soma + (e.servicos || []).length, 0);
...
function aplicarFiltrosExplore() {
  ...
  let filtradas = _empresasCache.filter(empresa => {
    ...
    if (industriasSel.length > 0 && !industriasSel.includes(empresa.industria)) {
      return false;
    }
    return true;
  });
...
async function inicializarExplore() {
  ...
  document.querySelectorAll('.filter-industria')
